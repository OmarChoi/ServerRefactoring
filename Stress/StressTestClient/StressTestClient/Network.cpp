...
const static int MAX_PACKET_SIZE = sizeof(unsigned short);
...
array<int, MAX_USER + MAX_NPC> client_map;
array<CLIENT, MAX_USER + MAX_NPC> g_clients;
...
{
	int psize = *(reinterpret_cast<unsigned short*>(packet));
...
void ProcessPacket(int ci, unsigned char packet[])
{
	...
	case SC_MOVE_OBJECT: {
		...
		if (move_packet->id < MAX_USER + MAX_NPC) {
	...
	break;
	default:
		int psize = *(reinterpret_cast<short*>(packet));
		cout << "Error Packet PacketSize: " << psize << endl;
...
void Adjust_Number_Of_Client()
{
	...
	if (active_clients >= MAX_USER) return;
	if (num_connections >= MAX_USER + MAX_NPC) return;
